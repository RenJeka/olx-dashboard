...
- [x] `web/src/components/SearchFiltersDrawer.tsx`: рядок-діапазон — `<HStack gap={2}>` →
  `<Flex direction={{ base: 'column', sm: 'row' }} gap={2} align={{ base: 'stretch', sm:
  'center' }}>`; `NativeSelect.Root` — `flex="1"` → `w={{ base: 'full', sm: 'auto' }}
  flex={{ sm: '1' }}` (full-width на мобільному, коли стек вертикальний); два
  `Input w="90px"` + кнопка видалення згруповані у вкладений `HStack gap={2}` (разом на
  одному рядку і на `base`, і на `sm+`).
- [x] `web/src/components/SearchActionPanel.tsx`: `SimpleGrid columns={3} gap={3}` →
  `columns={{ base: 2, md: 3 }}` (3 картки статистики затісні на 375px з 3 колонками).

## Група 5 — Майстер AI-аналізу: full-screen + картки на кроці 3

- [x] `AnalysisWizardDialog.tsx`: додано `const isMobile = useIsMobile();` —
  `DialogRoot size={isMobile ? 'full' : 'xl'}`, решта пропів (`placement`, `scrollBehavior`)
  без змін.
- [x] Степер: `<HStack gap={2}>` → `<HStack gap={2} wrap="wrap" rowGap={2}>`, 4 кроки
  переносяться на вузьких екранах без горизонтального скролу.
- [x] Крок 3: винесено спільні JSX-фрагменти (рендер-функції, без дублювання логіки
  `isIncluded`/`toggleIncluded`/evidence-highlight):
  - `renderPhotoTitle(l, fallbackId)` — фото (boxSize 12) + назва (`lineClamp={2}`).
  - `renderDescriptionBlock(l, desc, evidence)` — `DescriptionTooltip` + `HighlightText`;
    `lineClamp={isMobile ? 4 : 3}` (більше вертикального простору в картці на мобільному).
  - `renderCriteriaTags(r)` — `Wrap` бейджів з toggle/tooltip (логіка без змін).
  - Розгалуження рендеру за `isMobile`:
    - **desktop** (як було) — `Table.Root size="sm" css={{ tableLayout: 'fixed' }}` з
    ...
    - **mobile** — `Stack gap={3} maxH="60vh" overflowY="auto"` зі списком `Box p={3}
      borderWidth="1px" borderColor="border.subtle" rounded="md"` на `visibleRows`,
      всередині кожної картки — `Stack gap={2}>{photoTitle}{descriptionBlock}{criteriaTags}
      </Stack>` (вертикальний порядок: фото+назва → опис → теги).
...
- [x] `docs/architecture.md`: додано примітки про мобільну адаптацію — `hooks/useIsMobile.ts`
  (опис хука), `Searches.tsx` (overlay Drawer на мобільному), `Header.tsx` (responsive
  wrap/приховування заголовка), `components/analysis/` (full-screen wizard +
  desktop/mobile рендер кроку 3).
- [x] `docs/structure.md`: додано рядок для нового файлу `web/src/hooks/useIsMobile.ts`.

## Верифікація / test-cases

> Браузерна (Playwright) перевірка недоступна в цьому remote-середовищі (немає
> browser-тулів) — пункти нижче лишені `[ ]` як чекліст для ручної/Playwright-перевірки
> після мержу. Код-рев'ю + TS/vite build пройдені.
...
- [x] `npm run build` (web) проходить TS strict + vite build.
