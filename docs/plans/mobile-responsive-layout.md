...
- [ ] `web/src/components/table/topbar/BulkActionBar.tsx`: `ml={10}` →
  `ml={{ base: 0, sm: 10 }}`.
- [ ] `web/src/components/table/DescriptionTooltip.tsx`: `contentProps={{ maxW: '380px' }}`
  → `contentProps={{ maxW: { base: '85vw', md: '380px' } }}`.
- [ ] `web/src/components/table/topbar/ListingsFilterBar.tsx`: перевірити рендер
  `SegmentGroup.Root` (6 items) на 375px; якщо не переноситься коректно — обгорнути в
  `Box overflowX="auto"` (горизонтальний скрол замість ламання layout), без зміни
  функціоналу.
- [ ] `web/src/pages/ListingsTable.tsx`: без структурних змін — `Box flex="1" overflow="auto"`
  вже дає horizontal+vertical scroll для `Table.Root` (13 колонок, ~2200px); підтвердити
  плавність touch-скролу під час верифікації.
