...
- `theme/` — **система стилів** (єдина точка керування візуалом). `palette.ts` тримає
  `ACCENT_BASE` (базова Chakra-палітра акценту) і `STATUS_PALETTE`; `tokens.ts` через
  `defineConfig` будує токен `accent` — числову шкалу (`accent.50…950`) і семантичні аліаси
  (`accent.solid`/`fg`/`subtle`/…) на `ACCENT_BASE`, тож `colorPalette="accent"` і прямі
  відтінки `accent.500` поводяться як базова палітра, але керуються з одного рядка.
  ...
  Усі акцентні елементи використовують `colorPalette="accent"` (не зашите `"blue"`);
  `utils/status.ts` бере `STATUS_COLORS` з `theme/palette`. План — `plans/theme-and-constants-refactor.md`.
