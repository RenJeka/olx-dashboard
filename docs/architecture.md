...
| `scraper/olxCategories.ts` | Словник категорій OLX (`getCategoryMap`/`resolveCategoryPath`): дотягує дерево категорій (⚠️ ендпойнт `/api/v1/categories/` потребує живої верифікації), кешує в `server/data/olx-categories.json` (gitignored, TTL 1 доба, одна мережева спроба на процес), резолвить листовий `category_id` → шлях назв. Best-effort + graceful fallback на id/слаг. Живить дерево фільтра «Категорії» у `/filter-options`. |
...
| `GET` | `/api/searches/:id/filter-options` | ✅ Етап 2 — `{cities, sellers, pros, cons, categories}` для Drawer'а локальних фільтрів; `categories` — DISTINCT листові `category_id` пошуку зі шляхом назв зі словника OLX (лічильники рахує фронт у пам'яті) |
