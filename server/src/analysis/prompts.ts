 ...
 *
 * Два рівноправні шляхи виконання:
 *  - ВАРІАНТ А (агент із виконанням коду: Antigravity CLI, Claude Code) — згенерувати мапу
 *    «критерій → regex» у `patterns.json` і запустити готовий движок `analyze.py`, який
 *    масово сканує описи й створює `output.json`. LLM робить лінгвістику ОДИН раз, а
 *    детермінований скрипт — швидкий і стабільний масовий прохід.
 *  - ВАРІАНТ Б (звичайний чат без виконання коду) — опрацювати чанки напряму й повернути
 *    той самий об'єднаний JSON-масив.
 *
 * Обидва шляхи дають ОДНАКОВУ схему результату (див. matchingRulesAndFormat).
 */
export function buildManualZipInstructions(criteria: string[], mode: AnalysisMode): string {
  return [
    ...
    '',
    'Вміст пакета:',
    '- `analyze.py` — ГОТОВИЙ детермінований движок (Python, лише стандартна бібліотека). Його',
    '  логіку НЕ переписувати.',
    '- `descriptions/chunk-NNN.json` — вхідні оголошення, масив',
    '  {"id": <число>, "title": "...", "characteristics": "...", "description": "..."}.',
    '- `patterns.example.json` — приклад формату мапи критеріїв (НЕ використовувати як є).',
    '',
    '═══ ВАРІАНТ А — якщо ти агент, що вміє писати й запускати код (бажано) ═══',
    '',
    '1. Створи поруч з `analyze.py` файл `patterns.json` — JSON-обʼєкт, де КЛЮЧ — це ТОЧНИЙ',
    '   рядок критерію зі списку вище, а ЗНАЧЕННЯ — Python-сумісний regex, що ловить цей',
    '   критерій у тексті оголошення. Правила для regex:',
    '   - альтернація `|` синонімів УКРАЇНСЬКОЮ та РОСІЙСЬКОЮ + типовий сленг',
    '     (напр. «цегла», «донор», «вбитий»);',
    '   - морфологія через стем + `\\w*` замість перелічування всіх форм',
    '     (напр. `подряпин\\w*` ловить «подряпина/подряпини/подряпинами»);',
    '   - `\\b…\\b` (межі слова) для коротких/неоднозначних токенів, щоб уникнути збігів',
    '     усередині інших слів (напр. `\\bскол\\w*\\b`);',
    '   - НЕ додавай інлайн-прапорець `(?i)` — движок сам компілює з IGNORECASE',
    '     (інлайн `(?i)` не на початку патерну — помилка в Python 3.11+);',
    '   - НЕ описуй заперечення в regex — це робить движок (відкидає «без подряпин» тощо).',
    '2. Запусти движок: `python analyze.py` (або `python3 analyze.py`). Він сам прочитає',
    '   `patterns.json` і всі `descriptions/chunk-*.json`, відкине заперечені збіги, дістане',
    '   дослівні докази й створить `output.json`.',
    '3. Кінцевий результат — ЛИШЕ файл `output.json`. НЕ друкуй результат у консоль/відповідь;',
    '   достатньо створеного JSON-файлу.',
    '',
    '═══ ВАРІАНТ Б — якщо ти звичайний чат без виконання коду ═══',
    '',
    'Опрацюй КОЖЕН файл з `descriptions/` напряму й поверни ОДИН JSON-масив, що обʼєднує',
    'результати для всіх оголошень з усіх файлів (схема — нижче).',
    '',
    matchingRulesAndFormat(),
  ].join('\n');
}
