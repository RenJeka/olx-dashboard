...
Цей файл — ГОТОВИЙ движок. НЕ переписуй і НЕ редагуй його. Твоє єдине завдання як агента —
згенерувати поруч файл ``patterns.json`` з мапою «критерій -> regex» (див. prompt.txt),
після чого запустити цей скрипт РІВНО ОДИН раз. Результат — ``output.json`` поруч зі скриптом.
НЕ створюй жодних інших скриптів (scan/check/test/debug) і НЕ перевіряй/вдосконалюй патерни на
датасеті — це провалить завдання. Після успішного запуску одразу зупинись.
